...
samples, reactions, etc., almost directly as it states in the steps of your "hand written" original laboratory protocol.
  ...
  - :py:meth:`~Protocol.make_pre_mix`: *put together the components of a PreMixReagent by pipetting*
...
from .reagent import Reagent, PreMixReagent, MixReagent, PrimerReagent
from . import labware
from . import evo_mode as mode
            ...
            msg += " ({v:.1f} µL total) from [grid:{fg:d} site:{fs:d} {fw:s} " \
                   "into {to:s}[grid:{tg:d} site:{ts:d}] in order {do:s}:" \
                        .format(v  = reagent.min_vol(),
                                ...
                                fw = str([str(well) for well in reagent.aliquots]),
    ...
    def dilute_primer(self,
        ...
        with group(msg):
            msg += " into grid:{:d} site:{:d} {:s}:" \
                .format(labw.location.grid,
                        labw.location.site + 1,
                        str([str(well) for well in primer.aliquots]))
            instructions.comment(msg).exec()
            ctips = 1
            with self.tips(robot.mask_tips[ctips], tip_type=""):  # want to use preserved ?? selected=??
                ...
                msg = "   {v:.1f} µL from grid:{g:d} site:{st:d}:{w:s}" \
                      .format(v=volume,
                              g=labw.location.grid,
                              st=labw.location.site + 1,
                              w=str([str(well) for well in primer.diluent.aliquots]))
                instructions.comment(msg).exec()
                tips_type = self.robot.cur_arm().Tips[0].type  # only the 0 ??
                self.drop_tips(robot.mask_tips[ctips])
                self.get_tips(robot.mask_tips[ctips], tips_type)
